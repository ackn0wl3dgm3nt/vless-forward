...
read -rp "Entry port (на этой машине, UDP): " ENTRY_PORT
...
echo "Entry port (UDP): $ENTRY_PORT"
...
echo "[*] Adding UDP DNAT rule for port $ENTRY_PORT -> $BACKEND_IP:$BACKEND_PORT..."
iptables -t nat -A PREROUTING -p udp --dport "$ENTRY_PORT" -j DNAT --to-destination "$BACKEND_IP:$BACKEND_PORT"
...
echo "[*] Allowing forwarding for UDP port $ENTRY_PORT..."
iptables -A FORWARD -p udp --dport "$ENTRY_PORT" -j ACCEPT
...
echo "[+] Done. UDP $ENTRY_PORT -> $BACKEND_IP:$BACKEND_PORT"
